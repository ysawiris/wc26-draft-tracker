/* Goal alerts — watches consecutive Hub renders and announces what changed:
   goals (with the fantasy team that benefits), red cards (tiebreak points),
   and draft-order swings. Toasts stack bottom-right; an optional "notify"
   mode adds browser notifications plus a short synthesized goal horn when
   the tab is hidden. A bell button in the top nav cycles on → notify → off
   (persisted in localStorage "wc26.alerts"). The first render after page
   load only seeds the baseline snapshot, so opening the page never causes
   a toast storm. Everything fails soft: a broken alert never breaks the hub. */
...
(function () {
  ...
  var BOOT_QUIET_MS = 15000; /* silence diffs this long after load — the
    ...
    every goal since the last cron commit (up to ~10 min old) as breaking news */
  var TOAST_MS = 7000;
  ...
  function paintBell() {
    ...
    if (mode === "notify") {
      ...
      title = "Goal alerts: toasts + browser notifications + horn · tap to mute";
    } else if (mode === "off") {
      btn.textContent = "🔕";
      btn.classList.add("is-off");
      title = "Goal alerts: off · tap to turn on";
    } else {
      btn.textContent = "🔔";
      title = "Goal alerts: in-page toasts · tap to add browser notifications";
    }
    btn.title = title;
  ...
  function onBellClick() {
    try {
      if (mode === "on") {
        ...
        setMode("on");
        showToast("🔔 Alerts on", "Goal, red-card and draft-order toasts while the hub is open.", "info");
      }
    } catch (err) {
      console.error("Goal alerts bell failed:", err);
    }
  }
  ...
  function fxKey(fx) {
    return fx.group + "|" + fx.home.name + "|" + fx.away.name;
  }

  function snapFixture(fx) {
    return {
      hg: fx.homeGoals == null ? 0 : fx.homeGoals,
      ag: fx.awayGoals == null ? 0 : fx.awayGoals,
      rh: fx.cards && fx.cards.home ? (fx.cards.home.r || 0) : 0,
      ra: fx.cards && fx.cards.away ? (fx.cards.away.r || 0) : 0,
      counted: !!(INPLAY[fx.status] || FINISHED[fx.status])
    };
  }
  ...
  function buildSnapshot(ctx) {
    ...
    ctx.standings.forEach(function (row) {
      snap.ranks[row.team.abbr] = row.rank;
    });
    return snap;
  }
  ...
  function mergeHighWater(prev, snap) {
    var merged = { fixtures: {}, ranks: snap.ranks };
    Object.keys(snap.fixtures).forEach(function (key) {
      var cur = snap.fixtures[key];
      var old = prev.fixtures[key];
      merged.fixtures[key] = !old ? cur : {
  ...
  function diffRanks(prev, ctx) {
    var changes = [];
    ctx.standings.forEach(function (row) {
      var was = prev.ranks[row.team.abbr];
      if (typeof was === "number" && was !== row.rank) {
        changes.push({ abbr: row.team.abbr, from: was, to: row.rank });
      }
    });
    if (!changes.length) return null;
    changes.sort(function (a, b) { return a.to - b.to; });
    var parts = changes.map(function (c) {
      return c.abbr + (c.to < c.from ? " up to #" : " down to #") + c.to;
    });
    var crowned = null;
    changes.forEach(function (c) { if (c.to === 1) crowned = c; });
    if (crowned) {
      return { kind: "rank", tag: "rank",
        title: "👑 " + crowned.abbr + " takes the #1 pick!",
        sub: "New order: " + parts.join(", ") };
    }
    return { kind: "rank", tag: "rank", title: "📊 New order", sub: parts.join(", ") };
  }
  ...
  function diffRenders(prev, ctx) {
    ...
    var reds = [];
    var owners = {};
    try { owners = ctx.helpers.ownerByGroup() || {}; } catch (err) { owners = {}; }

    ctx.fixtures.forEach(function (fx) {
      if (fx.exhibition) return;
      var p = prev.fixtures[fxKey(fx)];
      if (!p) return;
      var cur = snapFixture(fx);
      /* Skip matches that went straight from "not started" to FINISHED —
         we were away the whole match, that's not breaking news. */
      if (!p.counted && FINISHED[fx.status]) return;
      var owner = owners[fx.group] || null;

      var dGoals = (cur.hg - p.hg) + (cur.ag - p.ag);
      if (dGoals > 0) {
        goals.push({ kind: "goal",
          tag: "goal-" + fxKey(fx) + "-" + cur.hg + "-" + cur.ag,
          title: "⚽ GOAL — " + fx.home.name + " " + cur.hg + "-" + cur.ag + " " + fx.away.name,
          sub: owner
            ? "Group " + fx.group + " · " + owner.name + " +" + dGoals
            : "Group " + fx.group + " · unclaimed group" });
        if (owner) flashUntil[owner.abbr] = Date.now() + FLASH_MS;
      }

      var dReds = (cur.rh - p.rh) + (cur.ra - p.ra);
      if (dReds > 0) {
        reds.push({ kind: "red",
          tag: "red-" + fxKey(fx) + "-" + (cur.rh + cur.ra),
          title: "🟥 Red card in Group " + fx.group,
          sub: owner
            ? "TB +" + (dReds * 2) + " for " + owner.name
            : "TB +" + (dReds * 2) + " · unclaimed group" });
      }
    });

    var events = goals.concat(reds);
    var rankEv = diffRanks(prev, ctx);
    if (rankEv) events.push(rankEv);
    return events;
  }
  ...
  function render(ctx) {
    try {
      ensureBell();
      ensureStack();
      if (!ctx || !ctx.fixtures || !ctx.standings) return;
      ...
      console.error("Goal alerts failed:", err);
    }
  }
